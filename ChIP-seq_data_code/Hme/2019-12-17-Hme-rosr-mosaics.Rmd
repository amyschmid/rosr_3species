---
title: "2019-12-17-Hme-rosr-mosaics"
output: html_document
---
---
title: "2019-10-25-Hca-rosr-mosaics"
output: html_document
editor_options: 
...
# Haloarcula hispanica ChIP-seq peak calling with mosaics
...
## Set up
```{r}
#Assumes that these packages are already installed (Hca analyzed first, see "2019-10-25-Hca-rosr-mosaics.Rmd" file for the details of how to install.)
library(mosaics)
library(hexbin)
library(tidyverse)
```
...
```{r}
sample_file <- read_csv("hme_sample_key.csv", col_names = F)
...
## Construct genomic bins for all samples, IPs - cap = 0
```{r}
# can change params according to dataset: fragLen, binSize, PET (paired end data) vs SET (single end data), can parallelize onto multiple cores (parallel = TRUE). Start with fragLen 200, then optimize once ChIP-QC is conducted (see below).
...
```{r}
#rep1 v 2
bin_rep<-readBins(type = c("chip","input"), fileName = c("mosaics/bins/CD4_sorted.bam_fragL200_bin200.txt", "mosaics/bins/CD6_sorted.bam_fragL200_bin200.txt"))
...
```{r}
#rep 2 v 3
bin_rep<-readBins(type = c("chip","input"), fileName = c("mosaics/bins/CD6_sorted.bam_fragL200_bin200.txt", "mosaics/bins/CD8_sorted.bam_fragL200_bin200.txt"))
...
```{r}
#rep 3 v 4
bin_rep<-readBins(type = c("chip","input"), fileName = c("mosaics/bins/CD8_sorted.bam_fragL200_bin200.txt", "mosaics/bins/CD10_sorted.bam_fragL200_bin200.txt"))
...
```{r}
#rep 1 v 4
bin_rep<-readBins(type = c("chip","input"), fileName = c("mosaics/bins/CD4_sorted.bam_fragL200_bin200.txt", "mosaics/bins/CD10_sorted.bam_fragL200_bin200.txt"))
...
```{r}
#rep 2 v 4
bin_rep<-readBins(type = c("chip","input"), fileName = c("mosaics/bins/CD6_sorted.bam_fragL200_bin200.txt", "mosaics/bins/CD10_sorted.bam_fragL200_bin200.txt"))
...
```{r}
#rep 1 v 3
bin_rep<-readBins(type = c("chip","input"), fileName = c("mosaics/bins/CD4_sorted.bam_fragL200_bin200.txt", "mosaics/bins/CD8_sorted.bam_fragL200_bin200.txt"))
...
```
Generally good reproducibility between reps, perhaps with the exception of rep 3 (higher count # relative to other reps).
...
for (i in 1:nrow(sample_file)) {
  ...
  export(peakTest, type="bed", filename=paste("mosaics/final_peaks/", sample_file$X3[i], ".bed", sep=""))
  export(peakTest, type="txt", filename=paste("mosaics/final_peaks/", sample_file$X3[i],  ".txt", sep=""))
}
```
Reps 1-3 look good - overall linearity between WCE and IP samples, with outliers in IP that represent potential peaks. Off-diagonal points not present in control sample as expected. Rep 4 something went wrong - lots of off-angle counts in WCE, suggesting spurious unevenness in coverage of WCE. Run overlap between peaks to see if these are filtered out b/c not present across bioreps.
